...
import pythoncom
from windows_services_model import WindowsServicesOut
from wmi import WMI, _wmi_namespace, _wmi_object

WAITING_TIME_SERVICE_RESPONSE_START = 3
WAITING_TIME_SERVICE_RESPONSE_STOP = 3


class WindowsService:
    def __init__(
        self, service_name: str = None, computer_name: str = None
    ) -> None:
        ...
        self.logger.debug(
            f'WINDOWS SERVICE: {self.service_name}. COMPUTER NAME: {computer_name}'
        )

        self.conn = self.__connect_wmi()
        self.object = self.__get_service_object()
        self.windows_service: WindowsServicesOut = (
            self.__windows_service_data()
        )

    def create(
        self, display_name: str, path_name: str, start_mode: str = 'auto'
    ) -> None:
        """
        ...
        if self.service_name is None:
            raise NoServiceNameDefined(
                'you must provide the service name you want to create'
            )
        ...
        try:
            ...
            process = subprocess.run(
                args=command, stdout=subprocess.PIPE, universal_newlines=True
            )
    ...
    def delete(self) -> None:
        ...
        if self.service_name is None:
            raise NoServiceNameDefined(
                'you must provide the service name you want to delete'
            )

        if self.__get_service_object() is None:
            raise ServiceDoesNotExists(
                'you cannot delete a service that does not exist'
            )
    ...
    def restart(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not self.is_running() and not self.is_stopped():
                raise UnknownState(
                    'the service is not ruuning and not stopped'
                )
        ...
        try:
            if self.is_running() and self.windows_service.accept_stop:
                ...
                if result[0] > 0:
                    if result[0] == 2:
                        raise NoPermission(
                            Messages.get_readable_message(result[0])
                        )
                    else:
                        raise Exception(
                            Messages.get_readable_message(result[0])
                ...
                if not self.is_stopped():
                    sleep(WAITING_TIME_SERVICE_RESPONSE_STOP)

                    if not self.is_stopped():
                        raise UnableToStop(
                            'Service is not stopped even after try to stop.'
                        )
    ...
    def start(self) -> Tuple[bool, str]:
        ...
        try:
            if not self.is_running():
                ...
                if not self.is_running():
                    sleep(WAITING_TIME_SERVICE_RESPONSE_START)

                    if not self.is_running:
                        raise UnableToStart(
                            'Service is not running even after try to start.'
                        )
    ...
    def get_info(self) -> WindowsServicesOut:
        """
        ...
        if self.computer_name is None:
            try:
                ...
                self.logger.exception(
                    'unable to connect to service on local machine'
                )
                raise UnableToConnectToComputer from err
        else:
            try:
                conn = WMI(computer=self.computer_name)
                self.logger.debug(
                    f'connected to service on remote machine: {self.computer_name}'
                )
            except Exception as err:
                self.logger.exception(
    ...
    def get_all_objects(self):
        ...
        try:
            win_objects = self.conn.Win32_Service()

            for win_object in win_objects:

                if not win_object or win_object.Name is None:
                    continue

                self.service_name = win_object.Name
                self.computer_name = self.computer_name

                data_object = self.__windows_service_data()
                list_objects.append(data_object)
    ...
    def __windows_service_data(self) -> WindowsServicesOut:
        """
        ...
        if self.object is None:
            return WindowsServicesOut(
                computer=self.computer_name, name=self.service_name
            )

        service_object = self.object
        windows_service = WindowsServicesOut(
...
class UnableToConnectToComputer(Exception):
